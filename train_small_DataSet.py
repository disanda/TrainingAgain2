...
sample_z = torch.rand((1, z_dim_num)).expand(sample_num, z_dim_num).to(device) # [-1,64]
test_z[:,:z_dim_num]=sample_z


##---------------z_d: 每个z_d一个one-hot位，每行一样---------
sample_z_d = torch.zeros((sample_num, c_d_num)).to(device) #[64,32]
for i in range(row):
...
sample_z_c = torch.zeros((sample_num, c_c_num)).to(device) #[64,32]
...
# update D2 network
		...
		#print(y_f.shape)
		d_t,_,_ = D(y)
		D_real = torch.sigmoid(d_t)
		d_f,_,_ = D(y_f)
		D_fake = torch.sigmoid(d_f)
		D_real_loss = BCE_loss(D_real, d_real_flag)#1
		D_fake_loss = BCE_loss(D_fake, d_fake_flag)#0
		D_loss = D_real_loss + D_fake_loss
		train_hist['D_loss'].append(D_loss.item())
...
# information loss
		...
		G_optimizer.zero_grad()
		y_info = G(z, c_c, c_d)
		d_f_3,D_disc_info,D_cont_info = D(y_info)
